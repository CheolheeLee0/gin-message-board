...
import (
	"gin-message-board/Struct"
	dao2 "gin-message-board/dao"
...
//列出一条留言
func (ms *MessageService) GetOneMsg(ctx *gin.Context, id string) error {
	thisDao := dao2.MessageDao{tool.GetDb()}
...
//列出所有留言
func (ms *MessageService) ListMsg(ctx *gin.Context) error {
	thisDao := dao2.MessageDao{tool.GetDb()}
